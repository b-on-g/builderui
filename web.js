...
;
	($.$bog_builderui_breadcrumbs) = class $bog_builderui_breadcrumbs extends ($.$mol_view) {
		items(){
			return [];
		}
		sub(){
	...
	($.$bog_builderui_breadcrumbs_item) = class $bog_builderui_breadcrumbs_item extends ($.$mol_view) {
		dom_name(){
			return "a";
		}
		title(){
		...
		attr(){
			return {
				...(super.attr()), 
				"href": (this.path()), 
				"bog_builderui_breadcrumbs_current": (this.current())
			};
		}
	};
...
(function ($) {
    ...
    (function ($$) {
        ...
        __decorate([
            $mol_mem
        ], $bog_builderui_breadcrumbs_item.prototype, "current", null);
        $$.$bog_builderui_breadcrumbs_item = $bog_builderui_breadcrumbs_item;
    })($$ = $.$$ || ($.$$ = {}));
})($ || ($ = {}));
...
;
	($.$bog_builderui_studio) = class $bog_builderui_studio extends ($.$bog_builderui_div) {
		...
		Breadcrumb_home(){
			const obj = new this.$.$bog_builderui_breadcrumbs_item();
			(obj.title) = () => ("Home");
			(obj.path) = () => ("home");
			return obj;
		}
		Breadcrumb_library(){
			const obj = new this.$.$bog_builderui_breadcrumbs_item();
			(obj.title) = () => ("Library");
			(obj.path) = () => ("home/library");
			return obj;
		}
		Breadcrumb_data(){
			const obj = new this.$.$bog_builderui_breadcrumbs_item();
			(obj.title) = () => ("Data");
			(obj.path) = () => ("home/library/data");
			return obj;
		}
		Breadcrumbs_demo(){
			const obj = new this.$.$bog_builderui_breadcrumbs();
			(obj.items) = () => ([
				(this.Breadcrumb_home()), 
				(this.Breadcrumb_library()), 
		...
		Breadcrumbs_card(){
			const obj = new this.$.$bog_builderui_card();
			(obj.sub) = () => ([(this.Breadcrumbs_title()), (this.Breadcrumbs_demo())]);
			return obj;
		}
		Code_title(){
			const obj = new this.$.$mol_view();
		...
		Namespace_field(){
			const obj = new this.$.$bog_builderui_field();
			(obj.hint) = () => ("bog (optional)");
			(obj.value) = (next) => ((this.namespace(next)));
			return obj;
		}
		app_name(next){
			if(next !== undefined) return next;
		...
		App_name_field(){
			const obj = new this.$.$bog_builderui_field();
			(obj.hint) = () => ("myapp");
			(obj.value) = (next) => ((this.app_name(next)));
			return obj;
		}
		Get_code_dialog(){
			const obj = new this.$.$bog_builderui_dialog();
			(obj.showed) = (next) => ((this.get_code_showed(next)));
			(obj.title) = () => ("Get Code");
			(obj.text) = () => ("Components will be exported as $<namespace>_<app>_*. Namespace can be empty.");
			(obj.confirm_title) = () => ("Download");
			(obj.close) = (next) => ((this.get_code_close(next)));
			(obj.confirm) = (next) => ((this.get_code_confirm(next)));
			(obj.content) = () => ([(this.Namespace_field()), (this.App_name_field())]);
			return obj;
		}
		plugins(){
			return [(this.Favicon())];
	...
	($mol_mem(($.$bog_builderui_studio.prototype), "Breadcrumbs_title"));
	($mol_mem(($.$bog_builderui_studio.prototype), "Breadcrumb_home"));
	($mol_mem(($.$bog_builderui_studio.prototype), "Breadcrumb_library"));
	($mol_mem(($.$bog_builderui_studio.prototype), "Breadcrumb_data"));
	($mol_mem(($.$bog_builderui_studio.prototype), "Breadcrumbs_demo"));
	($mol_mem(($.$bog_builderui_studio.prototype), "Breadcrumbs_card"));
	...
	($mol_mem(($.$bog_builderui_studio.prototype), "get_code_showed"));
	($mol_mem(($.$bog_builderui_studio.prototype), "get_code_close"));
	($mol_mem(($.$bog_builderui_studio.prototype), "get_code_confirm"));
	($mol_mem(($.$bog_builderui_studio.prototype), "namespace"));
	($mol_mem(($.$bog_builderui_studio.prototype), "Namespace_field"));
	($mol_mem(($.$bog_builderui_studio.prototype), "app_name"));
	($mol_mem(($.$bog_builderui_studio.prototype), "App_name_field"));
	($mol_mem(($.$bog_builderui_studio.prototype), "Get_code_dialog"));
...
`;
        class $bog_builderui_studio extends $.$bog_builderui_studio {
            ...
            async download_zip() {
                ...
                const prefix = `${full}_`;
                const rename = (src) => src
                    .replace(/bog_builderui_/g, prefix)
                    .replace(/'bog_builderui'/g, `'${full}'`);
                const script = document.querySelector('script[src$="web.js"]');
                const base = new URL(new URL(script.src).pathname.includes('/studio/-/') ? '../../' : 'bog/builderui/', script.src).href;
                const zip = new JSZip();
