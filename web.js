...
;
	($.$bog_builderui_studio) = class $bog_builderui_studio extends ($.$bog_builderui_div) {
		...
		Get_code_dialog(){
			...
			(obj.title) = () => ("Get Code");
			(obj.text) = () => ("Pick an app name — it becomes the $bog_<name>_* namespace inside the kit.");
			(obj.confirm_title) = () => ("Download");
			(obj.close) = (next) => ((this.get_code_close(next)));
			(obj.confirm) = (next) => ((this.get_code_confirm(next)));
			(obj.content) = () => ([(this.App_name_field())]);
			return obj;
		}
		plugins(){
			return [(this.Favicon())];
...
`;
        class $bog_builderui_studio extends $.$bog_builderui_studio {
            ...
            async download_zip() {
                ...
                const raw = this.app_name();
                const name = raw.toLowerCase().replace(/[^a-z0-9_]+/g, '_').replace(/^_+|_+$/g, '');
                if (!name)
                    return;
                const rename = (src) => src
                    .replace(/bog_builderui_/g, `bog_${name}_`)
                    .replace(/'bog_builderui'/g, `'bog_${name}'`);
                const zip = new JSZip();
